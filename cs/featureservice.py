...
def load_model():
    caffe_root = 'E:/caffe-master/'  # this file is expected to be in {caffe_root}/examples
    sys.path.insert(0, caffe_root + 'python')
    # plt.rcParams['figure.figsize'] = (10, 10)
    # plt.rcParams['image.interpolation'] = 'nearest'
    # plt.rcParams['image.cmap'] = 'gray'
    #  model = 'models/casiaface/casia.caffemodel'
    model = 'models/bvlc_alexnet/newResults/0601.caffemodel'
    ...
    caffe.set_mode_gpu()
    net = caffe.Net(caffe_root + 'models/bvlc_alexnet/deploy2.prototxt',
            caffe_root + model,
            caffe.TEST)
    transformer = caffe.io.Transformer({'data': net.blobs['data'].data.shape})
    transformer.set_transpose('data', (2, 0, 1))
    transformer.set_mean('data', np.load(caffe_root + 'models/bvlc_alexnet/newdata/newmean.npy').mean(1).mean(1))   # mean pixel.mean(1)
